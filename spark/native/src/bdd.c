...
static hash_matrix* create_G(nodei sz_l, nodei sz_r, char** _errmsg)  {
    ...
    if ( !(res = MALLOC(HASH_MATRIX_SZ(hash_sz,hash_sz))) ) {
        pg_error(_errmsg,"create_G: alloc fails, tried to alloc size: %lu, left treesize: %d, right treesize: %d, hash size: %d", HASH_MATRIX_SZ(hash_sz,hash_sz), sz_l, sz_r, hash_sz);
        return NULL;
    }
    res->sz_l    = sz_l;
    res->sz_r    = sz_r;
