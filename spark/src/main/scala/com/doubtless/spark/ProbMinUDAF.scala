...
object ProbMinUDAF
  ...
  val config = ConfigFactory.load().getConfig("com.doubtless.spark.prob-min")
  ...
  override def reduce(
    ...
    val newMap = b.map({ case (min, bdd) =>
      if (a._1.getOrElse(Double.NaN) < min.getOrElse(Double.NaN) || min.isEmpty)
        (min) -> (bdd & ~a._2)
      else if (a._1 == min) {
        newBddIncluded = true
