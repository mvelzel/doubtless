...
object HiveProbCountUDAF {
  class HiveProbCountEvaluator extends UDAFEvaluator {
    ...
    def iterate(bytes: BytesWritable) = {
      ...
      this.agg = agg.map({ case (count, bdd) =>
        agg get (count + 1) match {
          case Some(accBdd) => (count + 1) -> ((bdd & inputBdd) | (accBdd & ~inputBdd))
    ...
    def terminatePartial(): java.util.Map[IntWritable, BytesWritable] = terminate()
    ...
    def merge(inter: java.util.Map[IntWritable, BytesWritable]) = {
      ...
      val otherAgg = scalaInter.map({ case (count, bytes) => (count) -> new BDD(bytes.getBytes()) })

      this.agg = Map(0 to totalMax map { count =>
        (count ->
          (0 to count)
            .map(i => scalaInter get (new IntWritable(i)) match {
              case Some(bytes) => (this.agg getOrElse (count - i, BDD.False)) & (new BDD(bytes.getBytes()))
              case None => BDD.False
            })
    ...
    def terminate(): java.util.Map[IntWritable, BytesWritable] =
      this.agg.map({ case (count, bdd) => (new IntWritable(count)) -> new BytesWritable(bdd.buffer) }).asJava
  }
}
